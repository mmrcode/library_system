<?php
/**
 * Admin Messages - Library Management System
 * In-app messaging interface for administrators/librarians
 * 
        ...
        switch ($_POST['action']) {
            ...
            case 'broadcast_message':
                ...
                if ($subject && $message_body) {
                    // Get all students
                    $students = $messagingSystem->getStudents();
                    $sent_count = 0;
                    ...
                    foreach ($students as $student) {
                        ...
                        if ($result['success']) $sent_count++;
                    }
                    
                    $_SESSION['flash_message'] = "Broadcast message sent to {$sent_count} student(s).";
                    ...
                    <?php if ($view_message): ?>
                        ...
                        <div class="card">
                            ...
                            <div class="card-footer">
                                <button class="btn btn-primary" onclick="replyToMessage(<?php echo $view_message['message_id']; ?>, '<?php echo htmlspecialchars($view_message['sender_name']); ?>', '<?php echo htmlspecialchars($view_message['subject']); ?>', <?php echo $view_message['sender_id']; ?>)">
                        ...
                        <select class="form-select" name="receiver_id" id="receiver_id" required>
                            ...
                            <?php foreach ($students as $student): ?>
                                <option value="<?php echo $student['user_id']; ?>">
                                    <?php echo htmlspecialchars($student['full_name']); ?> (<?php echo htmlspecialchars($student['registration_number']); ?>)
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="broadcastModalLabel">
                    <i class="fas fa-bullhorn me-2"></i>Broadcast Message to All Students
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form method="POST">
                <div class="modal-body">
                    ...
                    <div class="alert alert-warning">
                        ...
                        <strong>Warning:</strong> This message will be sent to all active students in the system.
                    </div>
                    
                    <div class="mb-3">
                        <label for="broadcast_message_type" class="form-label">Message Type</label>
                        <select class="form-select" name="message_type" id="broadcast_message_type">
                            <option value="system">System Notification</option>
                            <option value="general">General Announcement</option>
                            <option value="book_issue">Book Issue Related</option>
                            <option value="fine">Fine Related</option>
                        </select>
                    </div>
                    
                    <div class="mb-3">
                        <label for="broadcast_subject" class="form-label">Subject</label>
                        <input type="text" class="form-control" name="subject" id="broadcast_subject" required>
                    </div>
                    
                    <div class="mb-3">
                        <label for="broadcast_message_body" class="form-label">Message</label>
                        <textarea class="form-control" name="message_body" id="broadcast_message_body" rows="6" required></textarea>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                    <button type="submit" class="btn btn-warning" onclick="return confirm('Are you sure you want to send this message to all students?')">
...
<script>
function replyToMessage(messageId, senderName, originalSubject, senderId) {
    // Set reply data
